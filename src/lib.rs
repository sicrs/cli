...
impl<T> App<T> {
    ...
    pub fn register_default(mut self, cmd: Command<T>) -> App<T> {
        self.default = cmd.flag(Flag::new("help", Some("h"), FlagKind::OptFlag, "help"));
        return self;
    }

    pub fn register(mut self, cmd: Command<T>) -> App<T> {
        self.cmds.push(cmd.flag(Flag::new("help", Some("h"), FlagKind::OptFlag, "help")));
        return self;
    }
                    ...
                    if arg[count].starts_with("--") {
                        ...
                        for flag in &cmd.flags {
                            if let Some(alias) = flag.alias {
                                if arg[count].contains(alias) {
                                    if flag.kind == FlagKind::InputFlag {
                                        if arg[count] == format!("-{}", alias) {
                                            if count + 1 == arg.len() || arg[count + 1].starts_with("-") {
...
mod tests {
    ...
    fn app_test() {
        let app: App<()> = App::new(()).register(
            ...
            .flag(Flag::new("input", Some("i"), FlagKind::InputFlag, "input"))
            .flag(Flag::new("output", Some("o"), FlagKind::OptFlag, "option")),
        );
